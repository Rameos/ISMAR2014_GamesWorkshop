...
public class PTAMTrailDrawer : MonoBehaviour {
	...
	protected Position2DListSink m_TrailsStart;
    protected Position2DListSink m_TrailsEnd;
	...
	void Start () {
        ...
        Component[] components = obj.GetComponents(typeof(Position2DListSink));
		
		if (components != null && components.Length > 0)
			foreach (Component comp in components)
			{
                Position2DListSink script = (Position2DListSink)comp;
	...
	void LateUpdate () {
        ...
        float factorwidth = Camera.mainCamera.GetScreenWidth() / CameraWidth;
        float factorheight = Camera.mainCamera.GetScreenHeight() / CameraHeight;
		...
		while (enumstart.MoveNext() && enumend.MoveNext())
		{
			...
			if (enumstart == null || enumend == null || Camera.mainCamera == null){
				Debug.Log("suff --------------------");
				continue;
			}
            float zPosition = Camera.mainCamera.nearClipPlane;
            position = new Vector3(enumstart.Current.x * factorwidth, (CameraHeight - enumstart.Current.y) * factorheight, zPosition);
			renderer.SetPosition (0, Camera.mainCamera.ScreenToWorldPoint(position));

            position = new Vector3(enumend.Current.x * factorwidth, (CameraHeight - enumend.Current.y) * factorheight, zPosition);
			renderer.SetPosition (1, Camera.mainCamera.ScreenToWorldPoint(position));
